## -*- coding: utf-8 -*-
import vk_api, random, time, wikipedia, requests, os, json
from vk_api.longpoll import VkLongPoll, VkEventType
from bs4 import BeautifulSoup
from vk_api import VkUpload

tokend = "91340e20eb9e1f3be9372382038210793d3eafa3739d111ad2f6f1c2f89a16234623b1c4955795d3205f0"
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        try:
            if event.from_chat:
                ...
                with open('iskl.json', encoding='utf-8') as f:
                    data = json.load(f)
                ...
                if msg in ["искл очистить"]:
                    if user == 400484262:
                        with open('iskl.json', encoding='utf-8') as f:
                            data = json.load(f)
                        data["iskl"] = []
                        with open('iskl.json', 'w', encoding='utf-8') as f:
            ...
            if event.to_me:
                ...
                if "e" in event.text.lower():
                    if event.text.split()[0] == "e":
                        msgd = msg.split(' ', 1)[1]
                        ...
                        for i in message:
                            crypt += chr(ord(i)^key)
                        message = crypt
                        sender(id, "encode: " + str(message))

                if "е" in event.text.lower():
                    if event.text.split()[0] == "е":
                        msgd = msg.split(' ', 1)[1]
                        key = 567  # Ключ шифрования
                        message = msgd
                        crypt = ''
                ...
                if "д" in event.text.lower():
                    if event.text.split()[0] == "д":
                        msgd = msg.split(' ', 1)[1]
                        key = 567  # Ключ шифрования
                        message = msgd
                        crypt = ''
